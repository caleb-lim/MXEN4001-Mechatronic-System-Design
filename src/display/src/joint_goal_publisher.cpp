...
class JointGoalPublisher {
    private:
        ros::Publisher dobot_joint_goal_pub;
    ...
    public:
        ...
        JointGoalPublisher(ros::NodeHandle *nh, std::string joint_topic, std::string dobot_joint_goal) {
            dobot_joint_goal_pub = nh->advertise<sensor_msgs::JointState>(dobot_joint_goal, 10);
            joint_pose_sub = nh->subscribe(joint_topic, 10, &JointGoalPublisher::publishStoredMessage, this);
        }
        ...
        void publishStoredMessage(const sensor_msgs::JointState& msg) {
            ...
            // ROS_INFO("GUI: [%f %f %f %f]", msg.position[0],  msg.position[1],  msg.position[2],  msg.position[3]);
            ...
            float visual_stable_joint;
            visual_joint_msg.position = {joint_1, joint_2, joint_3, visual_stable_joint};
            // //  Publish message
            ROS_INFO("Position: [%f %f %f %f]\n",joint_1, joint_2, joint_3, visual_stable_joint);
            std::cout << "Press Enter to publish joint state message" << std::endl;
            std::cin.ignore();
            dobot_joint_goal_pub.publish(visual_joint_msg);
        }
};
...
int main(int argc, char **argv)
{
    ...
    std::string visuals_topic = "/dobot/joint_goal";
   ...
   //  Check for topics in arguments
    ...
    JointGoalPublisher publisher = JointGoalPublisher(&nh, joint_topic, visuals_topic);
